...
const watchModelIDs = require('../Info/watchModelIDs.json');
...
async function CrawlOverModels() {
    const {browser, page} = await DefaultBrowser();
    ...
    while (!lastPageReached) { //as long as we don't get a 404 error
        ...
        for (let watchUrl of watchUrls) {
            await page.goto(watchUrl);
            await getWatchStats(page, db);
        }
    }
    console.log(c.green + '[+]' + c.reset + " Scraping Ended ! \n Closing browser...");
    await browser.close();
    await db.close();
}
